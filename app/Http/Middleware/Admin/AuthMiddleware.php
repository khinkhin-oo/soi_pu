    ...
    public function handle($request, Closure $next, $guard = null)
    {
        ...
        if($this->auth->user())
        {
            ...
            $notifications = $this->NotificationModel->with(['get_user_details'])
                                                 ->where(['receiver_id'=>'5e1408b57a6629112edafd2f', 'status'=>'0', 'receiver_type'=>'admin'])
                                                 ->orderBy('created_at','desc')
                                                 ->limit(5)
                                                 ->get()
                                                 ->toArray();

            $notification_count      = $this->NotificationModel->where(['receiver_id'=>'5e1408b57a6629112edafd2f', 'status'=>'0', 'receiver_type'=>'admin'])->count();
