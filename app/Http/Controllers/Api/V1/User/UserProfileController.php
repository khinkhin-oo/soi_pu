...
class UserProfileController extends Controller
{
    ...
    public function get_profile(Request $request)
	{
		...
		$obj_data = $this->UsersModel->where('_id',$user_id)->first();
		...
		if($obj_data)
		{
            ...
            return $this->build_response('SUCCESS','User details displayed successfully.',$arr_response['data']);
		}
		else
		{
	...
	{
		$arr_rules  = $arr_data = $arr_response = [];
		$user       = $this->auth->user();
		$user_id    = $user->id;
		...
		if($request->hasFile('profile_image'))
		{
			...
			{
				$arr_response['status']   = 'Error';
				$arr_response['msg']      = 'File type not supported.';
				$arr_response['data']     = [];
				return $arr_response;
			}
		}
		...
		$update = $this->UsersModel->where('_id',$user_id)->update($arr_data);
		if($update)
		{
			$obj_user = $this->UsersModel->where('_id',$user_id)->first();
			if($obj_user){
				$arr_user = $obj_user->toArray();
				$arr_user_details['id']	              = isset($arr_user['_id'])?$arr_user['_id']:'0';
				// $arr_user_details['first_name']       = isset($arr_user['first_name'])? $arr_user['first_name']:'';
				// $arr_user_details['last_name']        = isset($arr_user['last_name'])? $arr_user['last_name']:'';
                $arr_user_details['full_name']        = isset($arr_user['full_name'])? $arr_user['full_name']:'';
				$arr_user_details['email']            = isset($arr_user['email'])? $arr_user['email']:'';
				$arr_user_details['mobile_number']    = isset($arr_user['mobile_number'])? $arr_user['mobile_number']:'';
				$arr_user_details['address']          = isset($arr_user['address'])? $arr_user['address']:'';

				if(isset($arr_user['profile_image']) && !empty($arr_user['profile_image']) && file_exists($this->user_profile_image_base_img_path.$arr_user['profile_image']))
				{
				   $arr_user_details['profile_image']  =  $this->user_profile_image_public_img_path.$arr_user['profile_image']; 
				}
				else
				{
				   $arr_user_details['profile_image']  = ""; 
				}
			}

			$arr_response['data']     =  $arr_user_details;
			return $this->build_response('SUCCESS','Profile updated successfully.',$arr_response['data']);
		}
		else
		{
			return $this->build_response('ERROR','Error occur while updating profile.',[]);
		}
	}


	public function add_lawyer_profile(Request $request)
    {
    ...
    public function get_lawyer_profile(Request $request)
	{
		$arr_data   = $arr_response = [];
		$user       = $this->auth->user();
	...
	public function update_lawyer_profile(Request $request)
	{
    	$user       = $this->auth->user();
